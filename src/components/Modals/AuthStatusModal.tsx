...
export const AuthStatusModal: React.FC = () => {
  const { user, isWhitelisted, showStatusModal, setShowStatusModal } = useAuthStore()
  ...
  const getStatusInfo = () => {
    if (apiKey) {
      return {
        ...
        message: 'Local API Key detected. Using your personal Gemini key for all requests.',
        color: '#60a5fa',
        icon: '🔑'
      }
    }
    if (isWhitelisted) {
      return {
        ...
        message: 'Welcome! Your account is whitelisted. You are now using the real Gemini LLM via our secure proxy.',
        color: '#4ade80',
        icon: '🌟'
      }
    }
